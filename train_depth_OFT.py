"""
Output finetuning: This file will train in depth predictions rather than the model because its nearly 10x speedup in terms
of computation.
"""
...
class Depth_Estimation:
    ...
    def train(self):
        ...
        for iter, batch in enumerate(self.train_loader):
                ...
                for index in range(self.sequence_length):
                    ...
                    if self.args.MODEL.depth_network == "monodepth2":
                        ...
                        if self.args.OPTIMIZATION.refinement == "PFT" and self.args.LOSS.depth_regularizer and refine_step == 0:
                            self.initial_depths[("initial_depth", index, scale)] = inputs[
                                ("depth", index, scale)].copy().detach()
                        ...
                        if (
                                self.args.OPTIMIZATION.refinement == "PFT" or self.args.OPTIMIZATION.refinement == "OFT") and self.args.LOSS.depth_regularizer and refine_step == 0:
                            self.initial_depths[("initial_depth", index, scale)] = inputs[
                                ("depth", index, scale)].clone().detach()
            ...
            inputs[("depth", 1, 0)].requires_grad_().retain_grad()

            self.optimizer = define_optim(self.args, [inputs[("depth", 1, 0)]])
